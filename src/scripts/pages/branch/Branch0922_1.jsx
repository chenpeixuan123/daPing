...
/*var flage="a";
var titleIndex=0;*/
export default class Branch0922_1 extends Component {
    constructor(props) {
        super(props);
...
/*                Tools.fetchGet(
        ...
        self.sd;
        self.getODSData2Day();
        self.timer = setInterval(self.start, 5000);
        self.getZTData();
        self.getODSData1Day();
        self.getTDCurData();
        self.getaqdzbData();
...
//中间业务'http://192.168.43.151/cloud-monitor/data/load?dcId=2018&filter={"name":"SXFJYJSR"}',
    ...
    updateZjyw = (d)=>{
        let self = this;
        let zjywsj = d.datas;
        var everyArr = zjywsj.length > 0 ? zjywsj[0]["data"] : null;
        everyArr && everyArr.forEach(
            (obj) => {
                ...
                if(self.formatBranch[idname][sub+"新增"] == null){
                    self.formatBranch[idname][sub+"新增"] = [];
                }else{
                    self.formatBranch[idname][sub+"新增"].push(obj.value);
                }
        ...
        return (
            <div className={style.BranchIndex}>
                ...
                <div style={{
                    ...
                    <div
                        className={style1.title}>{det['名称']}</div>
                    <div style={{
                        position: 'absolute',
                        top: '86px',
                        left: '30px',
