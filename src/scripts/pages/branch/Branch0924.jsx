...
/*var flage="a";
var titleIndex=0;*/
export default class Branch0924 extends Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            detailsLine: {
                ...
                //净利润趋势分析
                ylTrend: {
                    yl: ['甘肃银行','甘肃银行1','甘肃银行2','甘肃银行2',],

                    time: ['123123','123123123','455666','88585588'],
                },
                // 营收趋势分析
                ysTrend: {
                    ys: [],
    ...
    start = () => {
        ...
        // let arrzc = new Array();
        // for(let i = 0;i < self.oneByOne.length;i++){
        //     arrzc.push(self.oneByOne[i].replace("(一级分支行)","") == "甘肃银行股份有限公司" ? self.oneByOne[i].replace("(一级分支行)",""):self.oneByOne[i].replace("(一级分支行)","").replace("甘肃银行股份有限公司",""))
        // }
        // this.fhName = arrzc;
...
//中间业务'http://192.168.43.151/cloud-monitor/data/load?dcId=2018&filter={"name":"SXFJYJSR"}',
    ...
    updateZjyw = (d)=>{
        ...
        let arr = d[0].ODSData;
        if(arr.length >= 2){
            let id = arr[0].subname.match(/\d+/g)[0];
            let sub = arr[0].subname.slice(id.length, obj.subname.length);
            // 格式化各分行的数据存储到对象中
            let idname = CodeToBank[id];
            self.formatBranch[idname][sub+"新增"] = arr[arr.length-1]-arr[arr.length-2]
        }
